...
class PpgStatusHistory {
	...
	public static final String XSD_TABLE_NAME = "ppg_status_history"

	public PpgStatusHistory(XMLStreamReader reader, boolean strict) {
						...
						if ( elementText ) {
							switch ( elementName ) {
								...
								case 'psu_id':
									...
									break
								case 'ppg_history_id':
									ppgHistoryId = elementText
									break
								case 'p_id':
									pId = elementText
									break
								case 'ppg_status':
									ppgStatus = elementText.toInteger()
									break
								case 'ppg_status_date':
									ppgStatusDate = elementText
									break
								case 'ppg_info_source':
									ppgInfoSource = elementText.toInteger()
									break
								case 'ppg_info_source_oth':
									ppgInfoSourceOth = elementText
									break
								case 'ppg_info_mode':
									ppgInfoMode = elementText.toInteger()
									break
								case 'ppg_info_mode_oth':
									ppgInfoModeOth = elementText
									break
								case 'ppg_comment':
									ppgComment = elementText
									break
	...
	static constraints = {
		...
		ppgStatus(nullable: false, inList:[1, 2, 3, 4, 5, 6, 7, -4])
		ppgStatusDate(nullable: true, maxSize:10, matches:"([0-9][0-9][0-9][0-9]-[0-9][0-9]-[0-9][0-9])?")
		ppgInfoSource(nullable: false, inList:[1, 2, 3, 4, -5, -4])
		ppgInfoSourceOth(nullable: true, maxSize:255)
		ppgInfoMode(nullable: false, inList:[1, 2, 3, 4, 5, 6, -5, -4])
