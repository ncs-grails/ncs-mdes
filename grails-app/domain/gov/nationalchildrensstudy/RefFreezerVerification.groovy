...
class RefFreezerVerification {
	...
	Integer correctionFactorTempUnit
	BigDecimal currentTemp
	Integer currentTempUnit
	BigDecimal maximumTemp
	Integer maximumTempUnit
	BigDecimal minimumTemp
	Integer minimumTempUnit
	String transactionType
	public static final boolean DOMAIN_CLASS_FROM_XSD = true
	public static final String XSD_TABLE_NAME = "ref_freezer_verification"

	public RefFreezerVerification(XMLStreamReader reader, boolean strict) {
						...
						if ( elementText ) {
							switch ( elementName ) {
								...
								case 'correction_factor_temp':
									...
									break
								case 'correction_factor_temp_unit':
									correctionFactorTempUnit = elementText.toInteger()
									break
								case 'current_temp':
									currentTemp = elementText.toBigDecimal()
									break
								case 'current_temp_unit':
									currentTempUnit = elementText.toInteger()
									break
								case 'maximum_temp':
									maximumTemp = elementText.toBigDecimal()
									break
								case 'maximum_temp_unit':
									maximumTempUnit = elementText.toInteger()
									break
								case 'minimum_temp':
									minimumTemp = elementText.toBigDecimal()
									break
								case 'minimum_temp_unit':
									minimumTempUnit = elementText.toInteger()
									break
	...
	static constraints = {
		...
		correctionFactorTempUnit(nullable: false, inList:[1, 2, -4])
		currentTemp(nullable: false)
		currentTempUnit(nullable: false, inList:[1, 2, -4])
		maximumTemp(nullable: false)
		maximumTempUnit(nullable: false, inList:[1, 2, -4])
		minimumTemp(nullable: false)
		minimumTempUnit(nullable: false, inList:[1, 2, -4])
	...
	static mapping = {
		...
		correctionFactorTempUnit column:'correction_factor_temp_unit'
		currentTemp column:'current_temp'
		currentTempUnit column:'current_temp_unit'
		maximumTemp column:'maximum_temp'
		maximumTempUnit column:'maximum_temp_unit'
		minimumTemp column:'minimum_temp'
		minimumTempUnit column:'minimum_temp_unit'
